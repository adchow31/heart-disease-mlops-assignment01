...
class PatientFeatures(BaseModel):
    """Input schema for a single patient's heart-disease risk features.

    Field descriptions follow the UCI Heart Disease (Cleveland) dataset dictionary.
    """
...
class PredictionResponse(BaseModel):
    ...
    label: str
    confidence: float = Field(..., description="Model's predicted probability for the predicted class")
    probability_disease: float = Field(..., description="Raw predicted probability of class 1")
    model_name: str
    model_version: str
